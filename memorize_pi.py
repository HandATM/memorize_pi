...
def get_start_from_user(best: int) -> int:
    if best == 0: return 0

    try:
        d = prompt(text = f'PI의 몇번째 자리에서 시작하시겠습니까? (0 ~ {best})', title = 'Memorize Pi의 내용:', default = '0')
        if d == None: return -1
        else: result = int(d)

    except: result = r.randrange(best + 1)
    if result not in range(best + 1): result = r.randint(0, best)
    return result
...
heart_point = 3
result = ''
dight = get_start_from_user(best_price)
if dight == -1: sys.exit()
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT: running = False
            ...
            elif event.key in range(48, 58): onclick(event.key - 48)()
